...
public class PokerroomGame extends XmlGame
{
	...
	/** looseness of the AI player (slides from N to LA) */
	private double						aiLooseness		= 0.0;

	/** type of AI to use */
	private int							aiType			= Player.AI_BAYES;

	/** whether we want to play real money FIXME */
	private boolean						doReal			= false;

	/** command to start the appletviewer */
	...
	{
		String logName = "PokerroomGame";
		Logger logger = Logger.getLogger(logName);
		logger.addHandler(new ConsoleHandler());
		try {
			logger.addHandler(new FileHandler("log/"+logName+"%u.log"));
		} catch (Throwable e) {
			logger.warning("Could not attach FileHandler" + e.getMessage());
		}
	}
	...
	private void getEmptyChairMap()
	{
		...
		for (int i = 0; i < 10; i++)
		{
			...
			Rect rect = table.getArea("chair" + (i + 1)).getRect(
					state.getRect(), ax, ay);
	...
	private boolean getDealerPos()
	{
		...
		while (--num > 0)
		{
			for (int i = 0; i < 10; i++)
			{
				Rect rect = table.getArea("tok" + (i + 1)).getRect(
						state.getRect(), ax, ay);
	...
	{
		try {
			logger.warning(msg +" dbg-" + shotCount + ".img");
			rect.write("screendump/dbg-" + (shotCount++) + ".img");
		} catch (IOException e) {
			logger.severe("Could not save screendump. " + e.getMessage() );
			e.printStackTrace();
		}
	}
